...
// include: C:\Users\Sbala\AppData\Local\Temp\tmpxlbglgfa.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'Relative_Simulator.data';
      var REMOTE_PACKAGE_BASE = 'Relative_Simulator.data';
...
Module['FS_createPath']("/", "resources", true, true);
    ...
    for (var file of metadata['files']) {
      ...
      async function processPackageData(arrayBuffer) {
        ...
        Module['removeRunDependency'](`fp ${name}`);
          ...
          Module['removeRunDependency']('datafile_Relative_Simulator.data');
      }
      Module['addRunDependency']('datafile_Relative_Simulator.data');
    ...
    loadPackage({"files": [{"filename": "/resources/dejavu-mono-latin-400-normal.ttf", "start": 0, "end": 340704}, {"filename": "/resources/displays.gif", "start": 340704, "end": 18810059}, {"filename": "/resources/keypress.mp3", "start": 18810059, "end": 18877768}, {"filename": "/resources/test.mp3", "start": 18877768, "end": 18944641}, {"filename": "/resources/test2.mp3", "start": 18944641, "end": 19011514}], "remote_package_size": 19011514});

  })();

// end include: C:\Users\Sbala\AppData\Local\Temp\tmpxlbglgfa.js
// include: C:\Users\Sbala\AppData\Local\Temp\tmpbfvugb3d.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: C:\Users\Sbala\AppData\Local\Temp\tmpbfvugb3d.js
// include: C:\Users\Sbala\AppData\Local\Temp\tmpuu4e8fyn.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: C:\Users\Sbala\AppData\Local\Temp\tmpuu4e8fyn.js
...
function findWasmBinary() {
  return locateFile('Relative_Simulator.wasm');
}
...
var ASM_CONSTS = {
 ...
 128302: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, window.miniaudio.unlock, true); }); window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 128606: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 128710: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 128881: ($0, $1, $2, $3, $4, $5) => { var deviceType = $0; var channels = $1; var sampleRate = $2; var bufferSize = $3; var pIntermediaryBuffer = $4; var pDevice = $5; if (typeof(window.miniaudio) === 'undefined') { return -1; } var device = {}; var audioContextOptions = {}; if (deviceType == window.miniaudio.device_type.playback && sampleRate != 0) { audioContextOptions.sampleRate = sampleRate; } device.webaudio = new (window.AudioContext || window.webkitAudioContext)(audioContextOptions); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; var channelCountIn = 0; var channelCountOut = channels; if (deviceType != window.miniaudio.device_type.playback) { channelCountIn = channels; } device.scriptNode = device.webaudio.createScriptProcessor(bufferSize, channelCountIn, channelCountOut); device.scriptNode.onaudioprocess = function(e) { if (device.intermediaryBufferView == null || device.intermediaryBufferView.length == 0) { device.intermediaryBufferView = new Float32Array(HEAPF32.buffer, pIntermediaryBuffer, bufferSize * channels); } if (deviceType == window.miniaudio.device_type.capture || deviceType == window.miniaudio.device_type.duplex) { for (var iChannel = 0; iChannel < channels; iChannel += 1) { var inputBuffer = e.inputBuffer.getChannelData(iChannel); var intermediaryBuffer = device.intermediaryBufferView; for (var iFrame = 0; iFrame < bufferSize; iFrame += 1) { intermediaryBuffer[iFrame*channels + iChannel] = inputBuffer[iFrame]; } } _ma_device_process_pcm_frames_capture__webaudio(pDevice, bufferSize, pIntermediaryBuffer); } if (deviceType == window.miniaudio.device_type.playback || deviceType == window.miniaudio.device_type.duplex) { _ma_device_process_pcm_frames_playback__webaudio(pDevice, bufferSize, pIntermediaryBuffer); for (var iChannel = 0; iChannel < e.outputBuffer.numberOfChannels; ++iChannel) { var outputBuffer = e.outputBuffer.getChannelData(iChannel); var intermediaryBuffer = device.intermediaryBufferView; for (var iFrame = 0; iFrame < bufferSize; iFrame += 1) { outputBuffer[iFrame] = intermediaryBuffer[iFrame*channels + iChannel]; } } } else { for (var iChannel = 0; iChannel < e.outputBuffer.numberOfChannels; ++iChannel) { e.outputBuffer.getChannelData(iChannel).fill(0.0); } } }; if (deviceType == window.miniaudio.device_type.capture || deviceType == window.miniaudio.device_type.duplex) { navigator.mediaDevices.getUserMedia({audio:true, video:false}) .then(function(stream) { device.streamNode = device.webaudio.createMediaStreamSource(stream); device.streamNode.connect(device.scriptNode); device.scriptNode.connect(device.webaudio.destination); }) .catch(function(error) { console.log("Failed to get user media: " + error); }); } if (deviceType == window.miniaudio.device_type.playback) { device.scriptNode.connect(device.webaudio.destination); } device.pDevice = pDevice; return window.miniaudio.track_device(device); },  
 131758: ($0) => { return window.miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 131831: ($0) => { var device = window.miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 132231: ($0) => { window.miniaudio.untrack_device_by_index($0); },  
 132281: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = window.miniaudio.device_state.started; },  
 132420: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; },  
 132560: () => { if (document.fullscreenElement) return 1; },  
 132606: () => { return Module.canvas.width; },  
 132638: () => { return parseInt(Module.canvas.style.width); },  
 132686: () => { document.exitFullscreen(); },  
 132713: () => { setTimeout(function(){ Module.requestFullscreen(false, false); }, 100); },  
 132785: () => { if (document.fullscreenElement) return 1; },  
 132831: () => { return Module.canvas.width; },  
 132863: () => { return screen.width; },  
 132888: () => { document.exitFullscreen(); },  
 132915: ($0) => { const canvasId = UTF8ToString($0); setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { document.querySelector(canvasId).style.width="unset"; }, 100); }, 100); },  
 133109: () => { return window.innerWidth; },  
 133135: () => { return window.innerHeight; },  
 133162: () => { if (document.fullscreenElement) return 1; },  
 133208: () => { return Module.canvas.width; },  
 133240: () => { return parseInt(Module.canvas.style.width); },  
 133288: () => { if (document.fullscreenElement) return 1; },  
 133334: () => { return Module.canvas.width; },  
 133366: () => { return screen.width; },  
 133391: () => { return window.innerWidth; },  
 133417: () => { return window.innerHeight; },  
 133444: () => { if (document.fullscreenElement) return 1; },  
 133490: () => { return Module.canvas.width; },  
 133522: () => { return screen.width; },  
 133547: () => { document.exitFullscreen(); },  
 133574: () => { if (document.fullscreenElement) return 1; },  
 133620: () => { return Module.canvas.width; },  
 133652: () => { return parseInt(Module.canvas.style.width); },  
 133700: () => { document.exitFullscreen(); },  
 133727: ($0) => { Module.canvas.style.opacity = $0; },  
 133765: () => { return screen.width; },  
 133790: () => { return screen.height; },  
 133816: () => { return window.screenX; },  
 133843: () => { return window.screenY; },  
 133870: () => { return window.devicePixelRatio; },  
 133906: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 133959: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 134010: () => { Module.canvas.style.cursor = 'none'; },  
 134047: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 134303: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 134354: () => { if (document.pointerLockElement) return 1; },  
 134401: () => { if (document.fullscreenElement) return 1; },  
 134447: () => { return window.innerWidth; },  
 134473: () => { return window.innerHeight; }
};
